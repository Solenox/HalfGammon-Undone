...
// Saves the current state of the board into the given arrays and bool reference
void HalfGammonBoard::getState(int x_pos[], int o_pos[], bool& is_x_turn) const {
	// Copy the entire xBoard and oBoard arrays into the passed in arrays
	for (int i = 0; i < ARRAY_SIZE; ++i) {
		x_pos[i] = xBoard[i];	// Save X's checker positions
		o_pos[i] = oBoard[i];	// Save O's checker positions
	}
	is_x_turn = xTurn;		// Save the current player's turn
}

// Restores the board to a previously saved state
void HalfGammonBoard::restoreState(const int x_pos[], const int o_pos[], bool is_x_turn) {
	// Copy the saved x and o positions back into the board arrays
	for (int i = 0; i < ARRAY_SIZE; ++i) {
		xBoard[i] = x_pos[i];	// Restore X's checker positions
		oBoard[i] = o_pos[i]; 	// Restore O's checker positions
	}
	xTurn = is_x_turn;		// Restore the player turn
}